...
const itemSchema = new mongoose.Schema({
  ...
  itemImage: { type: String, required: true } // Make sure this matches your field name
});
...
app.post('/items', upload.single('image'), async (req, res) => {
  try {
    const { itemName, startTime, endTime, amount, quantity, category } = req.body;
    const itemImage = req.file.path; // Ensure this matches your schema
    const newItem = await Item.create({ itemName, startTime, endTime, amount, quantity, category, itemImage });
    res.status(201).json({
      status: 'success',
      message: 'Item added successfully',
...
app.put('/items/:id', upload.single('image'), async (req, res) => { // Use 'image' to match your upload field
  try {
    const { itemName, startTime, endTime, amount, quantity, category } = req.body;
    const updateData = { itemName, startTime, endTime, amount, quantity, category };

    if (req.file) {
      updateData.itemImage = req.file.path; // Ensure this matches your schema
    }
